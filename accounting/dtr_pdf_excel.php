...
if (!validateSession($conn, 4)) { exit; } // Accounting only
...
// Get current user's name for export info
$userStmt = $conn->prepare("SELECT First_Name, Last_Name FROM users WHERE User_ID = ?");
$userStmt->execute([$_SESSION['user_id']]);
$userData = $userStmt->fetch(PDO::FETCH_ASSOC);
$userName = $userData ? $userData['First_Name'] . ' ' . $userData['Last_Name'] : "Accounting User";
...
switch ($action) {
    case 'pdf':
        ...
        <?php endforeach; ?>
        <div class="footer">
            Generated on <?php echo date('F j, Y \a\t g:i A'); ?> by <?php echo htmlspecialchars($userName); ?>
        </div>
        </body>
        </html>
        <?php
    ...
    default:
        ...
        try {
            ...
            // Add metadata
            $sheet->setCellValue('A2', 'Generated by: ' . $userName);
            $sheet->setCellValue('D2', 'Generated on: ' . date('F j, Y g:i A'));
            ...
            fputcsv($csvHandle, ["Generated by: $userName", "", "", "Generated on: " . date('F j, Y g:i A')]);
            $guards = guardsQuery($conn, $searchTerm, $locationFilter);
            foreach ($guards as $g) {
                $guardId = $g['User_ID'];
                $guardName = $g['First_Name'].' '.($g['middle_name']? $g['middle_name'].' ':'').$g['Last_Name'];
