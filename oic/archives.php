...
// Current OIC user id
$oicId = $_SESSION['user_id'];

// Get current OIC user's name
$hrStmt = $conn->prepare("SELECT First_Name, Last_Name FROM users WHERE Role_ID = 8 AND status = 'Active' AND User_ID = ?");
$hrStmt->execute([$oicId]);
$hrData = $hrStmt->fetch(PDO::FETCH_ASSOC);
$hrName = $hrData ? $hrData['First_Name'] . ' ' . $hrData['Last_Name'] : "OIC User";

// Get profile picture
$profileStmt = $conn->prepare("SELECT Profile_Pic, First_Name, Last_Name FROM users WHERE User_ID = ?");
$profileStmt->execute([$oicId]);
...
foreach ($roleMap as $roleId => $roleName) {
    ...
    // Count only users archived by this OIC
    $cnt = $conn->prepare("SELECT COUNT(*) FROM users WHERE Role_ID = ? AND archived_at IS NOT NULL AND archived_by = ?");
    $cnt->execute([$roleId, $oicId]);
    $total = (int)$cnt->fetchColumn();

    // Page data only users archived by this OIC
    $stmt = $conn->prepare("SELECT * FROM users WHERE Role_ID = :role AND archived_at IS NOT NULL AND archived_by = :arch ORDER BY Last_Name LIMIT :lim OFFSET :off");
    $stmt->bindValue(':role', $roleId, PDO::PARAM_INT);
    $stmt->bindValue(':arch', $oicId, PDO::PARAM_INT);
...
<body>
    ...
    <div class="main-content" id="main-content">
        <div class="header">
            ...
            <a href="profile.php" class="user-profile" id="userProfile" style="color:black; text-decoration:none;">
                ...
                // Attendance limited to those archived by this OIC via activity_logs pattern match
                $countSql = "SELECT COUNT(DISTINCT a.ID) FROM archive_dtr_data a 
                              LEFT JOIN users u ON u.User_ID = a.User_ID
                              LEFT JOIN guard_locations gl ON gl.user_id = a.User_ID AND gl.is_primary = 1
                              JOIN activity_logs al ON al.User_ID = :oic AND al.Activity_Type = 'Attendance Archive' AND al.Activity_Details LIKE CONCAT('%record ID ', a.ID, '%')" . $whereSql;
                $countStmt = $conn->prepare($countSql);
                $countStmt->bindValue(':oic', $oicId, PDO::PARAM_INT);
                ...
                $listSql = "SELECT DISTINCT a.ID, a.User_ID, a.first_name, a.last_name, u.employee_id, a.time_in, a.time_out, COALESCE(gl.location_name, '—') AS location_name
                            FROM archive_dtr_data a
                            LEFT JOIN users u ON u.User_ID = a.User_ID
                            LEFT JOIN guard_locations gl ON gl.user_id = a.User_ID AND gl.is_primary = 1
                            JOIN activity_logs al ON al.User_ID = :oic AND al.Activity_Type = 'Attendance Archive' AND al.Activity_Details LIKE CONCAT('%record ID ', a.ID, '%')" .
                            $whereSql . " ORDER BY a.time_in DESC LIMIT :lim OFFSET :off";
                $attStmt = $conn->prepare($listSql);
                $attStmt->bindValue(':oic', $oicId, PDO::PARAM_INT);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterUsers() {
            // If the generic archived user search controls are not present (OIC attendance-only view), skip.
        ...
        // Add event listeners for search (only if elements exist)
        if (searchInput) searchInput.addEventListener('input', filterUsers);
        if (searchButton) searchButton.addEventListener('click', filterUsers);
