...
// Validate session and require OIC role (8); handles redirect on failure
...
$currentYear = date('Y');
$currentMonth = date('m');
// Use server timezone explicitly (adjust if needed)
$today = new DateTime('now');
$day = (int)$today->format('d');
// Determine cutoff period based strictly on today's date
...
if (isset($_GET['filter_submit'])) {
    ...
    $searchTerm = '';
    $locationFilter = '';
    // Re-evaluate period only for current month/year context to ensure accuracy
...
$oicName = $oicData ? $oicData['First_Name'] . ' ' . $oicData['Last_Name'] : "OIC";
...
// Get OIC-assigned unique location names for filter dropdown
$oicUserId = $_SESSION['user_id'];
$locationsQuery = "
    SELECT DISTINCT gl.location_name
    FROM guard_locations gl
    INNER JOIN oic_locations ol
        ON ol.location_name = gl.location_name
       AND ol.oic_user_id = ?
       AND ol.is_active = 1
    WHERE gl.location_name <> ''
    ORDER BY gl.location_name
";
$locationsStmt = $conn->prepare($locationsQuery);
$locationsStmt->execute([$oicUserId]);
...
// Get list of guards that belong ONLY to OIC's assigned locations
$guardsQuery = "
...
// Build final parameters with OIC userId first
$finalParams = array_merge([$oicUserId], $params);
if (!empty($finalParams)) {
    foreach ($finalParams as $index => $param) {
...
<head>
    ...
    <title>OIC Daily Time Record - Green Meadows Security Agency</title>
...
<body>
    <!-- Sidebar -->
    <?php include_once '../global/oic_sidebar.php'; ?>
    ...
    <div class="container-fluid mt-4">
            ...
            <div id="attendanceRestrictionAlert" class="alert alert-warning mt-3">
                <i class="material-icons align-middle me-2">warning</i>
                You are viewing a past period (<?php echo htmlspecialchars($monthName.' '.$year); ?>). Adding new attendance records is only allowed for the current month (<?php echo date('F Y'); ?>).
            </div>
            <?php else: ?>
            <div id="attendanceRestrictionAlert" class="alert alert-warning mt-3 d-none">
                <i class="material-icons align-middle me-2">warning</i>
                Adding new attendance records is only allowed for the current month.
            </div>
            <?php endif; ?>
            ...
            <div id="guardAttendanceData">
                ...
                // Get list of guards with attendance data in the specified period, restricted to OIC-assigned locations
                $guardsQuery = "
                    SELECT DISTINCT u.User_ID, u.First_Name, u.Last_Name, u.middle_name, gl.location_name
                    FROM users u
                    INNER JOIN guard_locations gl
                ...
                if ($guardsStmt->rowCount() == 0) {
                    ...
                    while ($guard = $guardsStmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        // Shift column removed from UI; skip shift summary query to reduce overhead.
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        if ($attendanceStmt->rowCount() > 0) {
                                            // Prepare shift lookup once
                                            try { $getShiftStmt = $conn->prepare('SELECT shift_type FROM guard_schedules WHERE user_id = ? AND schedule_date = ? LIMIT 1'); } catch (Exception $e) { $getShiftStmt = null; }
                                            ...
                                            <tr>
                                                <td colspan="5" class="text-center">
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-success add-attendance mt-2" 
                                                            data-guard-id="<?php echo $guardId; ?>"
                                                            data-guard-name="<?php echo htmlspecialchars($guardName); ?>"
                                                            title="Add New Attendance">
   ...
   <!-- Mobile Bottom Navigation -->
    <div class="mobile-nav">
        <div class="mobile-nav-container">
            <a href="oic_dashboard.php" class="mobile-nav-item">
                <span class="material-icons">dashboard</span>
                <span class="mobile-nav-text">Dashboard</span>
            </a>
            <a href="attendance.php" class="mobile-nav-item active">
                <span class="material-icons">schedule</span>
                <span class="mobile-nav-text">Attendance</span>
            </a>
            <a href="scheduling.php" class="mobile-nav-item">
                <span class="material-icons">event_note</span>
                <span class="mobile-nav-text">Scheduling</span>
            </a>
...
$(document).ready(function() {
    console.log('Document ready - jQuery loaded successfully');
        // Global flag for month filter scope
        window.isCurrentMonthView = <?php echo ($month == $currentMonth && $year == $currentYear) ? 'true' : 'false'; ?>;
        if(!window.isCurrentMonthView){
            // Disable add attendance and import buttons visually for past months
            $('.add-attendance, .import-guard').prop('disabled', true).addClass('disabled');
        }
    ...
    $('#saveAttendanceBtn').click(function() {
        ...
        $.ajax({
            url: 'unified_attendance_process.php',
            type: 'POST',
            dataType: 'json',
            data: {
                action: 'edit',
    ...
    $(document).on('click', '.add-attendance', function() {
        ...
        if(!window.isCurrentMonthView){
            $('#attendanceRestrictionAlert').removeClass('d-none');
            return; // block modal open for past months
        }
    ...
    // Import attendance for a specific guard (open modal pre-filled)
    $(document).on('click', '.import-guard', function() {
        if(!window.isCurrentMonthView){
            $('#attendanceRestrictionAlert').removeClass('d-none');
            return; // block import for past months
        }
        const guardId = $(this).data('guard-id');
        const guardName = $(this).data('guard-name');
        $('#importGuardId').val(guardId);
        $('#importSelectedGuard').text(guardName);
        $('#importTemplateLink').attr('href', 'unified_attendance_process.php?action=download_template&guard_id=' + encodeURIComponent(guardId));
    ...
    $('#saveNewAttendanceBtn').click(function() {
        ...
        // Enforce current month for selected timeIn
        const currentYm = '<?php echo $currentYear . '-' . $currentMonth; ?>';
        if(timeIn.substring(0,7) !== currentYm){
            $('#attendanceRestrictionAlert').removeClass('d-none').html('<i class="material-icons align-middle me-2">warning</i> Attendance can only be added for the current month (<?php echo date('F Y'); ?>).');
            return;
        }
...
// Import Attendance Modal Handler
$(document).ready(function() {
    $('#importForm').on('submit', function(e) {
        ...
        if (!guardId) {
            Swal.fire({
                icon: 'warning',
                title: 'Select a Guard',
                text: 'Open the import modal using the Import button on a specific guard.',
                confirmButtonColor: '#2a7d4f'
            });
            return;
        }
        ...
        $('#importBtn').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>Uploading...');
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    ...
                    if (result.success) {
                        ...
                        let extra = '';
                        if (result && result.debug && Array.isArray(result.debug.rowErrors) && result.debug.rowErrors.length) {
                            const firstFew = result.debug.rowErrors.slice(0, 5).map(e => `Row ${e.row} [${e.code}]`).join('<br>');
                            extra = `<div style="margin-top:8px;">Details (first 5):<br>${firstFew}</div>`;
                        }
                        Swal.fire({
                            icon: 'error',
                            title: 'Import Failed',
                            html: (result.message || 'Import failed.') + extra,
                            confirmButtonColor: '#dc3545'
                        });
                    }
                } catch (e) {
            ...
            complete: function() {
                $('#importBtn').prop('disabled', false).html('<i class="material-icons me-1">upload</i> Upload');
            }
        });
    });
...
<div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #2a7d4f; color: white;">
            ...
            <div class="modal-body">
                <div class="alert alert-info mb-3">
                    ...
                    Importing attendance for: <strong id="importSelectedGuard">Select a guard via the Import button</strong>
                </div>
                <div class="alert alert-info">
                    <i class="material-icons align-middle me-2">info</i>
                    <strong>Instructions:</strong>
                    <ol class="mb-0 mt-2">
                        <li>Download the Excel template below (auto-filled for the selected guard)</li>
                        <li>Fill in the attendance data for guards</li>
                        <li>Upload the completed file</li>
                    </ol>
                </div>
                
                <div class="mb-3">
                    <a id="importTemplateLink" href="#" class="btn btn-outline-success w-100">
                ...
                <form id="importForm" enctype="multipart/form-data">
                    <input type="hidden" name="guardId" id="importGuardId" value="">
                    <div class="mb-3">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" form="importForm" class="btn btn-success" id="importBtn">
                    <i class="material-icons me-1">upload</i> Upload
                </button>
            </div>
        </div>
    </div>
